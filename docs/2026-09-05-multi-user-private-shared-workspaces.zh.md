...
- **复用 authorization seam 做登录。** [`dsh-credentials/authorization`](https://github.com/deepseek-ai/deepseek-harness/blob/master/packages/credentials/authorization/README.zh.md) 的 sign-in 流程是 session-bound 的(尝试存活于进程内,而登录时会话尚不存在)。它仍适合提供方登录;用户登录改用 carrier 路由。
