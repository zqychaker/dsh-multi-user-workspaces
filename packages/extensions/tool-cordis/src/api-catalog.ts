...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  {
    key: 'users',
    summary: 'Durable user registry.',
    description: 'Durable user registry. Startup opens the `users` domain, rebuilds the in-memory indexes, and validates that at most one user holds the owner bit. The dependency is the domain form alone; no session or persistence peer is needed.',
    methods: [
      {
        signature: 'async create(input: { username: string; password: string; displayName?: string }): Promise<User>',
        description: 'Create a user. Rejects an out-of-grammar username (`users/invalid-username`), a short password (`users/short-password`), and a case-insensitive username collision (`users/username-taken`). The first user ever created takes the owner bit.',
        parameters: [{ name: 'input', description: 'The username, password, and optional display name.' }],
        returns: 'the created user, projected without its password hash.',
      },
      {
        signature: 'async verify(username: string, password: string): Promise<User>',
        description: 'Verify credentials and resolve the user. Both an unknown username and a wrong password reject `users/bad-credentials` (one error, no timing oracle): a missing user still burns one scrypt derivation.',
        parameters: [{ name: 'username', description: 'The username, in any casing.' }, { name: 'password', description: 'The clear-text password.' }],
        returns: 'the resolved user, projected without its password hash.',
      },
      {
        signature: 'get(idOrUsername: string): User | undefined',
        description: 'Read one user by id or username, synchronously.',
        parameters: [{ name: 'idOrUsername', description: 'A user id or a username in any casing.' }],
        returns: 'the user, or `undefined` when neither names a stored user.',
      },
      {
        signature: 'list(): readonly User[]',
        description: 'Every user, in creation order (by `createdAt`, ties broken by username).',
        parameters: [],
        returns: 'a fresh ordered array of users, projected without password hashes.',
      },
      {
        signature: 'async remove(userId: UserId): Promise<void>',
        description: 'Remove a user and its record. An unknown id is an idempotent no-op; the owner rejects `users/owner-removal`. Workspaces and sessions are untouched.',
        parameters: [{ name: 'userId', description: 'The user to remove.' }],
